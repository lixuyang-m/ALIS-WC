...
class Runner(object):
    ...
    def training(self, global_weights, baseline_weights, curr_episode, env_params, seed=None):
        ...
        worker = Worker(self.metaAgentID, self.localNetwork, self.localBaseline,
                        curr_episode, self.device, save_img, seed, env_params)
    ...
    def evaluate(self, global_weights, seed, ltl_benchmark=None, eval_mode=None):
        ...
        print(f"  - 随机种子: {seed}")
        print(f"  - 动作选择模式: {'采样(Sampling)' if TrainParams.EVAL_USE_SAMPLING else '贪婪(Greedy/Argmax)'}")
        print(f"  - LTL评估开关: {'启用' if TrainParams.LTL_ENABLED_IN_EVALUATION else '禁用'}")
        print(f"  - 评估模式: {eval_mode if eval_mode else TrainParams.LTL_CONSTRAINT_TYPE}")
        ...
        if effective_ltl_benchmark:
            ...
            print(f"    - ⚠️  提供了 {len(ltl_benchmark)} 个约束，但已被 LTL_ENABLED_IN_EVALUATION=False 禁用")
        print(f"{'='*80}\n")
        ...
        worker = Worker(self.metaAgentID, self.localNetwork, self.localBaseline,
                        0, self.device, save_image=False, seed=seed, env_params=eval_env_params)
        ...
        _terminal_reward, _raw_return, _buffer, perf_metrics, _episode_truly_done = worker.run_episode(
            training=False,
        ...
        if TrainParams.LTL_ENABLED:
            print(f"  - LTL Overall Satisfaction: {perf_metrics.get('ltl_overall_satisfaction_rate', [0])[0]:.3f}")
        ...
        if total_actions > 0:
            ...
            print(f"  - UNLOAD: {unload_count:4d} ({unload_count/total_actions:6.1%})")
            print(f"  - REJECTED: {rejected_count:4d} ({rejected_count/total_actions:6.1%})")
        else:
            print(f"  - ⚠️  未执行任何动作！")
